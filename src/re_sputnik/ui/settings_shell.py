...
class SettingsShell(ctk.CTkFrame):
    ...
    def _build_nav(self) -> None:
        ...
        ctk.CTkLabel(head, text="Настройки", font=fonts.heading(), text_color=p.text).grid(
        ...
        for i, (key, label) in enumerate(_SECTIONS, start=1):
            ...
            btn = ctk.CTkButton(
                nav, text=label, font=fonts.body(), anchor="w",
        ...
        if self._on_exit is not None:
            ctk.CTkButton(
                nav, text="← Выход", font=fonts.body(), fg_color=p.fail,
    ...
    def _stub(self, title: str) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(frame, text=title, font=fonts.title(), text_color=self.p.text).grid(
            row=0, column=0, padx=32, pady=(40, 8), sticky="w"
        )
        ctk.CTkLabel(
            frame, text="Раздел в разработке.", font=fonts.body(), text_color=self.p.text_muted
