...
class AntiDPIScreen(ctk.CTkFrame):
    ...
    def __init__(self, master: ctk.CTkBaseClass, palette: Palette, client: RouterClient) -> None:
        ...
        ctk.CTkLabel(body, text="Обход DPI для сервисов, где не нужен полный VPN. "
...
class _ByeDPISection(ctk.CTkFrame):
    def __init__(self, master: ctk.CTkBaseClass, palette: Palette, client: RouterClient) -> None:
        ...
        self._status = ctk.CTkLabel(self, text="Считываю ByeDPI…", font=fonts.small(),
    ...
    def _render(self, d: dict[str, Any]) -> None:
        ...
        ctk.CTkButton(row, text="Сохранить стратегию", font=fonts.body(), fg_color=p.accent,
...
_GRP_REC = "··· Рекомендуемые ···"
_GRP_AUTO = "··· Полный набор ···"
...
class _ZapretSection(ctk.CTkFrame):
    def __init__(self, master: ctk.CTkBaseClass, palette: Palette, client: RouterClient) -> None:
        ...
        kit.SectionHeader(self, palette, "strategy", "Zapret2").grid(
            row=0, column=0, pady=(0, 2), sticky="w")
        ctk.CTkLabel(self, text="Также разблокирует видео (QUIC) и звонки, с которыми ByeDPI не справляется.",
                     font=fonts.small(), text_color=palette.text_muted, anchor="w").grid(
            row=1, column=0, sticky="w")
        self._status = ctk.CTkLabel(self, text="Считываю Zapret…", font=fonts.small(),
                                    text_color=palette.text_muted, anchor="w")
    ...
    def _render(self, d: dict[str, Any]) -> None:
        ...
        # Config card.
        self._enabled = ctk.StringVar(value="1" if cfg["enabled"] else "0")
        ctk.CTkSwitch(self._cfg_card, text="Включить Zapret", font=fonts.body(), variable=self._enabled,
                      onvalue="1", offvalue="0", progress_color=p.accent,
                      command=self._on_enable).grid(row=0, column=0, padx=16, pady=(12, 6), sticky="w")
        ...
        ctk.CTkLabel(self._cfg_card, text="Стратегия (опции nfqws2):", font=fonts.small(),
                     text_color=p.text_muted).grid(row=3, column=0, padx=16, sticky="w")
        self._cmd = ctk.CTkEntry(self._cfg_card, font=ctk.CTkFont(family="Consolas", size=12))
        self._cmd.insert(0, cfg["cmd_opts"])
        self._cmd.grid(row=4, column=0, padx=16, pady=(4, 6), sticky="ew")
        ...
        ctk.CTkSwitch(self._cfg_card, text="Звонки Discord через Zapret", font=fonts.body(),
        ...
        kit.SectionHeader(self._test_card, p, "strategy", "Тест стратегий").grid(
            row=0, column=0, padx=16, pady=(12, 2), sticky="w")
        ctk.CTkLabel(self._test_card, text="● = TLS-рукопожатие успешно, ○ = заблокировано. "
        ...
        self._test_btn = ctk.CTkButton(btnrow, text="▶ Тест текущей", font=fonts.body(),
                                       fg_color=p.accent, text_color=p.accent_fg, hover_color=p.accent_hover,
                                       command=self._run_test)
        self._test_btn.grid(row=0, column=0)
        self._full_btn = ctk.CTkButton(btnrow, text="⚡ Полная проверка", font=fonts.body(),
                                       fg_color=p.surface_hover, hover_color=p.border, command=self._full_start)
        self._full_btn.grid(row=0, column=1, padx=(8, 0))
        self._stop_btn = ctk.CTkButton(btnrow, text="■ Стоп", font=fonts.body(), fg_color=p.surface_hover,
    ...
    def _build_install_prompt(self) -> None:
        ...
        if not self._pkg_manager:
            ctk.CTkLabel(self._install_box, text="Менеджер пакетов не найден — установите zapret2 вручную.",
                         font=fonts.small(), text_color=p.fail, anchor="w").grid(row=0, column=0, sticky="w")
            return
        ctk.CTkLabel(self._install_box, text="Zapret (zapret2/nfqws2) не установлен. Установить сейчас?",
                     font=fonts.small(), text_color=p.warn, anchor="w").grid(
            row=0, column=0, columnspan=2, sticky="w", pady=(0, 4))
        self._install_btn = ctk.CTkButton(self._install_box, text="Установить Zapret", font=fonts.body(),
                                          fg_color=p.accent, text_color=p.accent_fg, hover_color=p.accent_hover,
    ...
    def _installed_cb(self, res: tuple[bool, str]) -> None:
        ...
        if ok:
            ...
            self._install_btn.configure(state="normal", text="Установить Zapret")
    ...
    def _install_err(self, e: BaseException) -> None:
        self._install_btn.configure(state="normal", text="Установить Zapret")
        self._install_note.configure(text=f"Ошибка: {e}", text_color=self.p.fail)
    ...
    def _run_test(self) -> None:
        opts = self._cmd.get().strip()
        if not opts:
            self._save_note.configure(text="Сначала выберите или введите стратегию.", text_color=self.p.warn)
            return
        self._test_btn.configure(state="disabled", text="Тестирую…")
        self._clear_out()
        client = self._client
        run_async(self, lambda: zp.run_test(client, opts), self._show_test, self._test_err)

    def _test_err(self, e: BaseException) -> None:
        self._test_btn.configure(state="normal", text="▶ Тест текущей")
        self._err(e)
    ...
    def _full_start(self) -> None:
        if self._full_running:
            return
        if not self._cands:
            self._full_note.configure(text="Список стратегий недоступен.", text_color=self.p.warn)
            return
    ...
    def _full_next(self, i: int) -> None:
        ...
        if self._full_stop or i >= n:
            ...
            return
        cand = self._cands[i]
        self._full_note.configure(text=f"Проверка {i + 1}/{n}: {cand['name']}", text_color=self.p.text_muted)
        client = self._client
    ...
    def _full_row(self, i: int, res: dict[str, Any]) -> None:
        ...
        if ok:
            ctk.CTkButton(line, text="Применить", font=fonts.small(), width=92, fg_color=p.accent,
    ...
    def _apply_full(self, args: str, name: str) -> None:
        ...
        run_async(self, lambda: zp.set_cmd_opts(client, args),
                  lambda _r: self._save_note.configure(
                      text=f"Применено: {name} — нажмите «Применить изменения».", text_color=self.p.ok),
                  self._err)
    ...
    def _full_finish(self, stopped: bool) -> None:
        ...
        self._full_note.configure(
            text=f"{'Остановлено' if stopped else 'Готово'} — пройдено {self._full_passed}/{len(self._cands)}",
