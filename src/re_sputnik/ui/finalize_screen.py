...
class FinalizeScreen(ctk.CTkFrame):
    def __init__(self, master: ctk.CTkBaseClass, palette: Palette, client: RouterClient,
        ...
        self._sc = kit.WizardScaffold(self, palette, step=8, label="Завершение", footer=False)
        self._scroll = self._sc.content
        b = self._scroll

        ctk.CTkLabel(b, text="Завершение", font=fonts.title(), text_color=palette.text).grid(
            row=0, column=0, pady=(28, 2), padx=32, sticky="w")
        ctk.CTkLabel(b, text="Несколько необязательных рекомендаций.", font=fonts.body(),
        ...
        kit.SectionHeader(hn, palette, "network", "Имя роутера").grid(
            row=0, column=0, padx=16, pady=(12, 2), sticky="w")
        ctk.CTkLabel(hn, text="Как роутер называется в сети и в веб-интерфейсе. Сейчас обычно "
        ...
        kit.SectionHeader(zr, palette, "resource", "zram-swap (рекомендуется)").grid(
            row=0, column=0, padx=16, pady=(12, 2), sticky="w")
        ctk.CTkLabel(zr, text="Сжатый swap в ОЗУ — снижает риск нехватки памяти на устройствах с "
        ...
        self._zram_status = ctk.CTkLabel(zr, text="Проверяю…", font=fonts.small(),
                                         text_color=palette.text_muted)
        self._zram_status.grid(row=2, column=0, padx=16, pady=(4, 2), sticky="w")
        self._zram_btn = ctk.CTkButton(zr, text="Установить zram-swap", font=fonts.body(),
                                       fg_color=palette.accent, text_color=palette.accent_fg, hover_color=palette.accent_hover,
        ...
        ctk.CTkLabel(ap, text="Дополнительные приложения LuCI (необязательно)",
        ...
        for i, (pkg, title, desc) in enumerate(fin.OPTIONAL_APPS):
            ...
            ctk.CTkLabel(rowf, text=title, font=fonts.body(), text_color=palette.text,
                         anchor="w").grid(row=0, column=0, sticky="w")
            ctk.CTkLabel(rowf, text=desc, font=fonts.small(), text_color=palette.text_muted,
                         anchor="w", wraplength=440, justify="left").grid(row=1, column=0, sticky="w")
            btn = ctk.CTkButton(rowf, text="Установить", font=fonts.body(), width=120,
                                fg_color=palette.accent, text_color=palette.accent_fg, hover_color=palette.accent_hover,
                                command=lambda p=pkg: self._install_app(p))
            btn.grid(row=0, column=1, rowspan=2, padx=(8, 0))
            self._app_btns[pkg] = btn
        ctk.CTkLabel(ap, text="После установки оба приложения требуют активации и дополнительной "
        ...
        kit.SectionHeader(pv, palette, "security", "Приватность устройства").grid(
            row=0, column=0, padx=16, pady=(12, 2), sticky="w")
        ctk.CTkLabel(pv, text="Применено для нового устройства, чтобы роутер не выдавал себя "
        ...
        self._dhcp_switch = ctk.CTkSwitch(
            pv, text="Не отправлять имя устройства в DHCP-запросах", font=fonts.body(),
        ...
        self._ntp_switch = ctk.CTkSwitch(
            pv, text="Российский пул NTP (0–3.ru.pool.ntp.org)", font=fonts.body(),
        ...
        ctk.CTkButton(b, text="Готово", font=fonts.heading(), height=42, fg_color=palette.ok,
    ...
    def _render(self, d: dict[str, Any]) -> None:
        ...
        for pkg, installed in (d.get("apps") or {}).items():
            if installed and pkg in self._app_btns:
                self._app_btns[pkg].configure(state="disabled", text="Установлено ✓")
        ...
        if z.installed:
            self._zram_status.configure(
                text="Установлен" + (" и активен ✓" if z.active else " (не активен)"),
                text_color=self.p.ok if z.active else self.p.warn)
            self._zram_btn.configure(state="disabled", text="Уже установлен")
        else:
            self._zram_status.configure(text="Не установлен.", text_color=self.p.text_muted)
    ...
    def _install_app(self, pkg: str) -> None:
        ...
        btn.configure(state="disabled", text="Устанавливаю…")
        self._status.configure(text=f"Устанавливаю {pkg} (+ русский язык)…",
                               text_color=self.p.text_muted)
        client = self._client
        run_async(self, lambda: fin.install_luci_app(client, pkg),
                  lambda ok: self._app_done(pkg, ok), lambda e: self._app_err(pkg, e))

    def _app_done(self, pkg: str, ok: bool) -> None:
        btn = self._app_btns[pkg]
        if ok:
            btn.configure(state="disabled", text="Установлено ✓")
            self._status.configure(text=f"{pkg} установлено.", text_color=self.p.ok)
        else:
            btn.configure(state="normal", text="Установить")
            self._status.configure(text=f"Не удалось установить {pkg} (нет в фиде или нет связи).",
                                   text_color=self.p.warn)

    def _app_err(self, pkg: str, exc: BaseException) -> None:
        self._app_btns[pkg].configure(state="normal", text="Установить")
        self._status.configure(text=f"Ошибка установки {pkg}: {exc}", text_color=self.p.fail)

    def _rename(self) -> None:
        name = self._host.get().strip()
        if not name:
            self._host_status.configure(text="Введите имя роутера.", text_color=self.p.warn)
            return
        self._host_btn.configure(state="disabled", text="Применяю…")
        self._host_status.configure(text="Переименовываю роутер…", text_color=self.p.text_muted)
        client = self._client
        run_async(self, lambda: ov_engine.set_hostname(client, name), self._renamed, self._rename_err)

    def _renamed(self, name: str) -> None:
        self._host_btn.configure(state="normal", text="Переименовать")
        self._host_status.configure(text=f"Готово — роутер переименован в «{name}».",
                                    text_color=self.p.ok)

    def _rename_err(self, exc: BaseException) -> None:
        self._host_btn.configure(state="normal", text="Переименовать")
        self._host_status.configure(text=f"Не удалось переименовать: {exc}", text_color=self.p.fail)
    ...
    def _toggle_dhcp(self) -> None:
        ...
        run_async(self, lambda: fin.set_dhcp_hostname_hidden(client, on),
                  lambda _r: self._status.configure(
                      text="Имя устройства в DHCP " + ("скрыто." if on else "восстановлено."),
                      text_color=self.p.ok),
                  lambda e: self._status.configure(text=f"Не сохранено: {e}", text_color=self.p.fail))
    ...
    def _toggle_ntp(self) -> None:
        ...
        run_async(self, lambda: fin.set_ru_ntp(client, on),
                  lambda _r: self._status.configure(
                      text="NTP: " + ("российский пул." if on else "пул OpenWrt."),
                      text_color=self.p.ok),
                  lambda e: self._status.configure(text=f"Не сохранено: {e}", text_color=self.p.fail))

    def _install_zram(self) -> None:
        self._zram_btn.configure(state="disabled", text="Устанавливаю…")
        self._status.configure(text="Устанавливаю zram-swap…", text_color=self.p.text_muted)
        client = self._client
        run_async(self, lambda: fin.install_zram(client), self._zram_done,
                  lambda e: (self._zram_btn.configure(state="normal", text="Установить zram-swap"),
    ...
    def _zram_done(self, z: fin.ZramStatus) -> None:
        if z.installed:
            self._zram_status.configure(
                text="Установлен" + (" и активен ✓" if z.active else " (не активен)"),
                text_color=self.p.ok if z.active else self.p.warn)
            self._zram_btn.configure(text="Уже установлен")
            self._status.configure(text="zram-swap установлен.", text_color=self.p.ok)
        else:
            self._zram_btn.configure(state="normal", text="Установить zram-swap")
            self._status.configure(text="Установка не подтвердилась.", text_color=self.p.warn)
