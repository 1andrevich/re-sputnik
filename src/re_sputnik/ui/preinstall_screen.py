...
class PreinstallScreen(ctk.CTkFrame):
    def __init__(self, master: ctk.CTkBaseClass, palette: Palette, client: RouterClient,
        ...
        ctk.CTkLabel(body, text="Предустановить пакеты", font=fonts.title(),
                     text_color=palette.text).grid(row=0, column=0, pady=(28, 2), padx=32, sticky="w")
        ctk.CTkLabel(body, text="Пакеты скачает этот компьютер и передаст на роутер — "
        ...
        self._banner = ctk.CTkLabel(body, text="Проверяю, что уже установлено на роутере…",
        ...
        ctk.CTkLabel(core_card, text="Ядро (с модулями ядра)", font=fonts.heading(),
                     text_color=palette.text).grid(row=0, column=0, padx=16, pady=(12, 4), sticky="w")
        for i, (val, label) in enumerate((("hiddify", "hiddify-core"),
                                          ("singbox", "sing-box-extended")), start=1):
        ...
        ctk.CTkLabel(core_card, text="Обязательно: kmod-nft-tproxy + kmod-tun.", font=fonts.small(),
                     text_color=palette.text_muted).grid(row=3, column=0, padx=16, pady=(2, 12), sticky="w")
        ...
        ctk.CTkSwitch(app_card, text="Установить LuCI-приложение Re:HomeProxy (+ русский язык)",
                      ...
                      progress_color=palette.accent).grid(row=0, column=0, padx=16, pady=(12, 2),
                                                          sticky="w")
        ctk.CTkLabel(app_card, text="Без него на роутере будет только ядро, но не сам HomeProxy "
        ...
        self._continue_btn = ctk.CTkButton(
            body, text="Далее: интернет (WAN) и Wi-Fi →", font=fonts.heading(), height=42,
    ...
    def _load_status(self) -> None:
        ...
        run_async(self, lambda: install_app.software_status(client), self._render_status,
                  lambda _e: self._banner.configure(
                      text="Не удалось проверить роутер — можно предустановить заново.",
        ...
        if st.ready:
            ...
            self._banner.configure(
                text=f"✓ На роутере уже есть: приложение, {core_label}, модули ядра{extra}. "
                     ...
                     else f"✓ На роутере уже всё установлено ({core_label}{extra}).",
                text_color=self.p.ok)
            self._go.configure(text="Предустановить заново")
            ...
            self._banner.configure(
                text="Частично установлено: " + ", ".join(have) +
                     ". Предустановка доставит недостающее.", text_color=self.p.warn)
        else:
            self._banner.configure(text="На роутере ничего не установлено.",
                                   text_color=self.p.text_muted)
    ...
    def _run(self) -> None:
        if self._busy:
            return
        self._busy = True
        self._go.configure(state="disabled", text="Работаю…")
    ...
    def _err(self, e: BaseException) -> None:
        self._busy = False
        self._go.configure(state="normal", text="Скачать и установить")
    ...
    def _done(self, res: preinstall.PreinstallResult) -> None:
        ...
        self._go.configure(state="normal", text="Скачать и установить")
        if res.ok:
            self._append("✓ Установлено: " + ", ".join(res.installed))
            if self._on_continue is not None:
                self._append("Можно заранее настроить интернет (WAN) и Wi-Fi — кнопка ниже.")
                self._continue_btn.grid()
        else:
